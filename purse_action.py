...
class PurseAction(ActionParser[AgentID, np.ndarray, np.ndarray, GameState, int]):
    """
    ...
    def parse_actions(self, actions: Dict[AgentID, np.ndarray], state: GameState, shared_info: Dict[str, Any]) -> Dict[AgentID, np.ndarray]:
        ...
        for agent, action in actions.items():
            ...
            if state.cars[agent].physics.position[0] < 0:
                parsed_actions[agent] = self._lookup_table[action] * [1, -1, 1, -1, -1, 1, 1, 1]
            else:
